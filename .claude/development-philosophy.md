...
**For this learning project specifically:**
...
### For Production Code (when lessons graduate to `src/`)
...
- **CQRS (Command Query Responsibility Segregation)**: Separate reads from writes when complexity justifies it
...
### Productionize Phase (Lessons → `src/`)
...
- What's the current bottleneck? (TOC)
...
- **Doc Norton**: Emergent design, organic architecture ([docondev.com](https://docondev.com))
