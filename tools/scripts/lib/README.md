...
4. **Archive Models** (`tools/services/archive/models.py`)
...
```bash
# Dry run
uv run python tools/scripts/reprocess_normalized_tags.py --dry-run --limit 5
...
```python
from tools.scripts.lib.metadata_transformers import BaseTransformer
from tools.scripts.lib.transform_versions import get_version
