"""Integration test for tag normalizer using SurrealDB (not Qdrant).

Requires remote services:
- Ollama: http://192.168.16.241:11434 (qwen2.5:7b model)
- SurrealDB: Configured via environment
- Infinity: http://192.168.16.241:7997 (for embeddings)

Run with: uv run pytest compose/services/tests/integration/test_surrealdb_tagger_normalizer.py -v -s
"""
...
retrieval augmented generation (RAG) using a vector database like SurrealDB or Pinecone.
...
@pytest.fixture
def retriever(infinity_url):
    """Create SurrealDB retriever using remote Infinity service."""
    return create_surrealdb_retriever(
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_normalizer_with_surrealdb_context(ollama_url, retriever):
    """Test Phase 2: Normalization with semantic context from SurrealDB."""
    import os

    os.environ["OLLAMA_URL"] = ollama_url

    normalizer = create_normalizer(model="ollama:qwen2.5:7b", retriever=retriever)

    print("\n=== Getting Semantic Context from SurrealDB ===")
    ...
    print("\n=== Phase 2: Normalize ===")
    normalized = await normalizer.normalize_metadata(raw, context_tags=context)
...
async def test_normalizer_with_vocabulary(ollama_url, vocabulary):
    ...
    normalizer = create_normalizer(model="ollama:qwen2.5:7b", vocabulary=vocabulary)
    ...
    normalized = await normalizer.normalize_metadata(
        raw, vocabulary_tags=vocabulary.get_all_tags()
    )
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_normalizer_full_pipeline_surrealdb(ollama_url, retriever, vocabulary):
    """Test full normalize_from_transcript pipeline with SurrealDB."""
    import os
    ...
    print("\n[OK] Full pipeline test complete with SurrealDB!")
