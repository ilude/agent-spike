...
# Default URLs - use environment variables for flexibility
LOKI_URL = os.getenv("LOKI_URL", "http://192.168.16.241:3100")
PROMETHEUS_URL = os.getenv("PROMETHEUS_URL", "http://192.168.16.241:9090")
TEMPO_URL = os.getenv("TEMPO_URL", "http://192.168.16.241:3200")
...
class LGTMClient:
    ...
    def __init__(
        ...
        Args:
            loki_url: Loki API endpoint (default: LOKI_URL env var or GPU server)
            prometheus_url: Prometheus API endpoint (default: PROMETHEUS_URL env var)
            tempo_url: Tempo API endpoint (default: TEMPO_URL env var)
            timeout: Request timeout in seconds
        """
        self.loki_url = (loki_url or LOKI_URL).rstrip("/")
        self.prometheus_url = (prometheus_url or PROMETHEUS_URL).rstrip("/")
        self.tempo_url = (tempo_url or TEMPO_URL).rstrip("/")
